...
export class MessageService {
  ...
  async send(did, message) {
    const encryptedMessage = await this._networkMember.createEncryptedMessage(
      did,
      message
    );
  ...
  async getAll() {
    ...
    return Promise.all(
      messages.map(async ({ id, fromDid, createdAt, message }) => {
        const decrypted = await this._networkMember.readEncryptedMessage(
          message
        );
  ...
  async execute(path, method, token, data) {
